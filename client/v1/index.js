...
const communities = deals.reduce((accumulator, deal) => {
  const key = deal.community;
  // If the key doesn't exist yet, initialize it with an empty array
  if (!accumulator[key]) {
    accumulator[key] = [];
  }
  // Push the current deal into the correct array
  accumulator[key].push(deal);
  return accumulator;
}, {});
// 2. Log the variable
console.log('TODO 8');
...
for (const community in communities) {
  // Sort in-place: b.price - a.price for descending order
...
let hasVeryOldItems = false;
const now = new Date();
const threeWeeksInMs = 1000 * 60 * 60 * 24 * 21;
for (const item of VINTED) {
  const datePublished = new Date(item.published);

  const ageInMs = now - datePublished;

  if (ageInMs > threeWeeksInMs) {
...
const dealersAsString = JSON.stringify(MY_FAVORITE_DEALERS);
localStorage.setItem('favoriteDealers', dealersAsString);
// 2. log the localStorage
console.log('LAST TODO');
console.log('Saved in localStorage:', localStorage.getItem('favoriteDealers'));